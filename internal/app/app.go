...
func NewDefaultRelayer(
	...
	txSender, err := submit.NewTxSender(ctx, neutronClient, codec.Marshaller, *cfg.NeutronChain, keybase, keyName, logRegistry.Get(TxSenderContext))
	if err != nil {
	...
	neutronChain, targetChain, err := loadChains(cfg, keybase, keyName, logRegistry)
	if err != nil {
...
func loadChains(cfg config.NeutronQueryRelayerConfig, keybase keyring.Keyring, keyName string, logRegistry *nlogger.Registry) (neutronChain *cosmosrelayer.Chain, targetChain *cosmosrelayer.Chain, err error) {
	...
	neutronChain, err = relay.GetNeutronChain(logRegistry.Get(NeutronChainProviderContext), cfg.NeutronChain, keyName)
